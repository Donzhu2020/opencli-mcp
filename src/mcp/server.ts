...
export function createMcpServer(rt: Runtime, sessionId: string, opts: { version?: string } = {}): SessionServer {
  ...
  server.registerTool('tab_network', { title: 'Network capture', description: 'op:start arms capture (optionally filtered by URL substring); op:read returns captured requests (or performance entries when capture was not armed).', inputSchema: { tab: z.string().optional(), op: z.enum(['start', 'read']), pattern: z.string().optional(), limit: z.number().int().default(100), includeStatic: z.boolean().default(false) } }, async ({ tab, op, pattern, limit, includeStatic }) => run(async () => { const t = await tabOf(tab); return ok(op === 'start' ? { tab: t.id, started: await t.network.start(pattern ?? '') } : { tab: t.id, entries: await t.network.read({ pattern, limit, includeStatic }) }); }));
