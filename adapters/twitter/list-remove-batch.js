...
import { runListRemove } from './list-remove.js';
...
export default defineAdapter({
  description: 'Remove multiple users from an X list you own, from a comma-separated username list.',
  access: 'write',
  domain: 'x.com',
  args: [
    { name: 'list_id', type: 'string', required: true, help: 'Numeric ID of a list you own (from the `lists` command)' },
    { name: 'usernames', type: 'string', required: true, help: 'Comma-separated screen names to remove (with or without @)' },
    { name: 'interval', type: 'int', default: 5, help: 'Seconds to wait between removals' },
  ],
  async run({ tab, args }) {
    ...
    for (let i = 0; i < usernames.length; i++) {
      try { rows.push(await runListRemove(tab, listId, usernames[i])); }
