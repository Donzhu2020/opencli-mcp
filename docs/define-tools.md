...
- Network-first: if a JSON endpoint of the site was captured (`tab.network.start()` in `js`), the tool becomes one `page.fetchJson` call through the logged-in page.
...
- `inputs` are explicit: `{query: "what you typed"}` or `{query: {sample: "what you typed", description: "search terms", type: "string", mode: "exact"|"within"}}`. By default only whole literals equal to the sample become `args.query`; declare `mode: "within"` when the value also appears inside longer literals you want parameterized (the text you typed into a bigger string, or a checkpoint like `Results for <query>`). Nothing is guessed.
