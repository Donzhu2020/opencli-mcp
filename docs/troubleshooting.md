...
- `dialog_open`: a native JavaScript dialog is blocking the tab. `tab.dialog.get()/accept()/dismiss()` (in `js`) read and answer it. `no_dialog`: nothing is pending (dialogs are tracked only while the debugger is attached, so one opened before the first command may need a `tab_evaluate` to surface).
