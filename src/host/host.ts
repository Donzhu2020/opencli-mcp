...
export async function runNativeHost(opts: { version: string }): Promise<void> {
  ...
  const state = () => ({ pid: process.pid, port: http.port, host: http.host, token, startedAt: rt.startedAt, extensionVersion: bridge.extensionVersion, protocolVersion: bridge.protocolVersion, contextId: bridge.contextId, version: opts.version });
  bridge.on('hello', (h) => { log(`extension ${h.extensionVersion} ${bridge.connected ? 'connected' : 'rejected'} (protocol ${h.protocolVersion})`); writeHostState(state()); });
