...
describe('tools_compile: frozen flows run on the agent engine with checkpoints', () => {
  it('emits act steps with replay selectors, expect checkpoints, explicit args and structured step failures', () => {
    ...
    expect(strict.func).toContain('page.expect({"text":"Results for red shoes","url":"/search?q=red"})'); // exact by default: a checkpoint is never parameterized by guessing
  ...
  it('prefers a captured JSON endpoint', () => {
    ...
    expect(exact.func).toContain('page.fetchJson("https://shop.example/api/items?q=red")');
