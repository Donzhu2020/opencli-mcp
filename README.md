...
|---|---|---|
| **typed tools** | `tab_open`, `tab_observe`, `tab_act`, `tab_find`, `tab_screenshot`, `tab_wait`, `tab_evaluate`, `tab_network`, `tab_claim`, `session_name`, `session_finalize`, `sites_search`, `sites_enable` (adds `<site>_<command>` tools), `site_run`, `recon_discover`, `tools_define`, `tools_compile`, `capabilities_enable`, `cdp_send`, `docs_get`, `doctor` | hosts that gate/render per tool; simple flows |
...
`tab_act` takes one target — `{ref}` from the last observe, `{role,name}`, `{label}`, `{text}`, `{testid}`, `{css,nth?}` or `{x,y}` — waits until it is actionable, dispatches real input, and returns `matches_n` / `match_level` plus branchable error codes (`not_found`, `stale_ref`, `selector_ambiguous`, `intercepted`, `not_visible`, `timeout`). `tab_observe` returns a diff when the page changed only a little.
