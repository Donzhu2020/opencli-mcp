...
try { const keyFile = resolve(process.env.OPENCLI_MCP_HOME ?? resolve(process.env.HOME ?? '', '.opencli-mcp'), 'extension-key.json'); const { key } = JSON.parse(readFileSync(keyFile, 'utf8')); if (key) manifest.key = key; } catch { /* not installed yet */ }
