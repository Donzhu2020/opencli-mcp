...
function adapt(page: IPage, session: string, surface: 'browser' | 'adapter', close: () => Promise<void>): RuntimePage {
  ...
  for (const [k, v] of Object.entries(extras)) if (obj[k] === undefined || ['session', 'surface', 'getActivePage', 'setActivePage', 'closeWindow', 'closeTab', 'act', 'engineEvaluate', 'dialog'].includes(k)) obj[k] = v;
