...
- `policy` (off by default) — `askNewOrigins` makes the first navigation to a new host return `needs_origin_approval` until `session.allowOrigin(host)` is called in `js`; `confirmWrites` gates write site commands: `site_run` and the `<site>_<command>` tools ask the user through the client (a multi-round-trip approval prompt, no confirm flag); `allowedHosts` pre-approves hosts (in memory, for the run).
