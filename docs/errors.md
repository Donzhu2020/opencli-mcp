...
| navigation | `invalid_url`, `page_not_loaded`, `stale_page`, `page_not_in_session`, `page_released`, `tab_create_failed`, `tab_create_cleanup_failed`, `no_tab` | the URL was blocked/unreachable, the tab is gone, or finalize handed it back to the user; open or claim a fresh tab. If cleanup failed, use `tab_list` and `tab_close` on the remaining tab |
