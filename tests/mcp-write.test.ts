...
async function harness() {
  ...
  await client.callTool({ name: 'tools_define', arguments: { site, name: 'poke', description: 'test write', access: 'write', browser: false, args: [{ name: 'x', required: false }], func: 'async () => ({ done: true })' } });
...
describe('write commands execute directly', () => {
  ...
  it('still rejects invalid write arguments', async () => {
    const h = await harness();
    try {
      await h.client.callTool({ name: 'tools_define', arguments: { site: h.site, name: 'need', description: 'needs text', access: 'write', browser: false, args: [{ name: 'text', required: true }], func: 'async () => { throw new Error("must not execute"); }' } });
