...
6. `tab_observe` returns a diff by default when the page changed only a little; pass `diff:false` for the full state. Sources: `dom` (budgeted text with `[N]` refs and compound form hints), `aria` (Playwright's accessibility snapshot with `[ref=eN]` refs — pass `{ref:"e12"}` to tab_act), `ax` (raw accessibility tree).
