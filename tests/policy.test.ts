...
describe('policy shapes', () => {
  ...
  it('asks for new origins and confirmations when enabled', () => {
    const p = new Policy({ askNewOrigins: true, confirmWrites: true, allowedHosts: ['example.com'], blockedHosts: ['evil.test'] });
    ...
    expect(p.checkOrigin('https://evil.test/')).toMatchObject({ allowed: false, code: 'site_blocked', retryable: false });
