...
| authorization | `needs_origin_approval` (+`retryable`), `needs_confirmation`, `user_declined`, `cdp_not_allowed`, `read_docs_first` (+`docs`) | `session.allowOrigin(host)` for origins; typed write tools ask the user via an approval prompt (no confirm flag); inside `js` re-call with `confirm:true` after approval; `user_declined` = the user said no; read the named doc |
