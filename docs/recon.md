...
Candidates are evidence, not contracts: the scan cannot prove auth, signatures, pagination or field semantics. Verify a candidate with `tab_evaluate`/`js` (`page.fetchJson`) or by triggering the UI with `tab_network` armed, then freeze it with `tools_define`. Never persist suspected secret values; the analyzer reports only redacted previews.
