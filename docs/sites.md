...
- `sites.enable(site, {write?})` in `js` — load a site's commands as typed tools `<site>_<command>` (read-only by default; `write:true` adds write commands). Emits tools/list_changed. `sites_disable` removes them.
