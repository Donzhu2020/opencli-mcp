...
import { NATIVE_HOST_NAME, PROTOCOL_VERSION } from '../protocol.js';
...
export interface DoctorResult {
  ...
  host: { stateFile: string; running: boolean; port?: number; backend?: string; extensionConnected?: boolean; extensionVersion?: string | null; protocolVersion?: number | null; expectedProtocolVersion: number; error?: string };
...
export async function doctor(): Promise<DoctorResult> {
  ...
  else if (state?.protocolVersion != null && state.protocolVersion !== PROTOCOL_VERSION) advice.push(`Extension protocol ${state.protocolVersion} does not match host protocol ${PROTOCOL_VERSION}. Update the Chrome Web Store extension and npm package, then run doctor again.`);
  ...
  return {
    ...
    manifests,
    host: { stateFile: HOST_STATE_FILE, running: health.ok, port: state?.port, backend: health.backend, extensionConnected: health.extensionConnected, extensionVersion: state?.extensionVersion, protocolVersion: state?.protocolVersion, expectedProtocolVersion: PROTOCOL_VERSION, error: health.error },
    chromeRunning,
    advice,
  };
}
