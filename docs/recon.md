...
Candidates are evidence, not contracts: the scan cannot prove auth, signatures, pagination or field semantics. Verify a candidate in `js` (`tab.evaluate` for a read, or `page.fetchJson` inside a site command) or by triggering the UI with capture armed (`tab.network.start()` in `js`), then freeze it with `tools_define`.
