...
Candidates are evidence, not contracts: the scan cannot prove auth, signatures, pagination or field semantics. Verify a candidate in `js` (`tab.evaluate` for a read, or `page.fetchJson` inside a site command) or by triggering the UI with `tab_network` armed, then freeze it with `tools_define`. Never persist suspected secret values; the analyzer reports only redacted previews.
