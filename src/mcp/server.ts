...
export function createMcpServer(rt: Runtime, sessionId: string, opts: { version?: string } = {}): SessionServer {
  ...
  server.registerTool('tab_observe', {
    ...
    inputSchema: { tab: z.string().optional(), mode: z.enum(['state', 'screenshot', 'both']).default('state'), source: z.enum(['dom', 'ax', 'aria']).default('dom').describe('dom: budgeted text with [N] refs; ax: accessibility tree; aria: Playwright aria snapshot with [ref=eN] refs usable as act targets'), diff: z.boolean().default(true), interactive: z.boolean().optional().describe('only interactive elements'), maxTextLength: z.number().int().optional(), maxDepth: z.number().int().optional(), annotate: z.boolean().default(false).describe('overlay [N] labels on the screenshot'), fullPage: z.boolean().default(false) },
